...
## Per-learner navigation — every account arranges its own bar — DONE (2026-08-28)

The top bar was six links hardcoded in **two** places — `layouts/public.blade.php` for the reader shell, the Breeze `layout.navigation` Volt component for the dashboard shell — and they had already drifted: the public bar carried Courses, Gyms and My courses; the dashboard bar carried three links and never learned about the other three. Making order and visibility a per-account preference forced that duplication out first, because a preference that only half the app honours is worse than no preference at all. Verified by `NavPreferencesTest` (146 tests pass total) + `e2e/nav-preferences.spec.ts` (12 e2e specs pass), light and dark.

- **The bar became data, with one owner**: `App\Support\Navigation` is now the only thing that knows the links exist — same idiom as `Palette`, for the same reason. Both shells render from `Navigation::for($user)`, so hiding *Gyms* hides it everywhere instead of on one page out of two. `users.nav_preferences` (nullable JSON, `{"order": [...], "hidden": [...]}`) holds the arrangement; null means untouched.
- **Two axes, deliberately not collapsed into one**: `audience` (`all` / `auth` / `staff`) decides who is ever *offered* an item — a permission-shaped fact — and `preference` reorders and hides within that set. Both lists are scrubbed against the audience set on save, so a hand-posted key can neither conjure an Admin link nor survive a role change.
- **Hides are stored as an opt-OUT list, not a show-list.** The tempting shape is "save what the user wants to see", and it quietly breaks the next slice: a link added to `ITEMS` next year would be missing for exactly the users engaged enough to have saved an arrangement. Storing hides instead means new links appear by default and land at the end of a customised bar.
- **`dashboard` is locked** — movable, never hideable. The public bar has no user dropdown, so it is the only route back to the form that edits the bar; without the lock a learner can customise themselves into a bar with no way home. It renders "always shown" where the other rows render a Show switch.
- **The auth links moved out of the right-hand group.** *My courses / Dashboard / Admin* used to sit `ml-auto`; ordering is meaningless across two separated groups, so they joined the main row and the right side now holds only log-in/register and the theme toggle.
- **Arrows, not drag-and-drop**: no sortable dependency, works on touch, and it is drivable by name from Playwright (`Move Gyms earlier`) — a drag test would have asserted pixels instead of behaviour.
- **A cross-component repaint costs a second round trip, and the first e2e run hid it.** Saving dispatches `nav-updated`, which `layout.navigation` listens for — but "Saved." lands on the save response while the bar refreshes on the *next* one. The original assertion read `allTextContents()` immediately and passed once by luck, then failed under parallel load. Fixed as an auto-retrying locator assertion, not a sleep. The same spec also resets before it reorders: it drives the shared dev account, and the moves assume a known starting order.
- Deliberately not done: **guests keep the defaults** — there is no account to hang a preference on. The natural extension is a `localStorage` layer reordering the same registry client-side, and it can be added without touching `Navigation`.
